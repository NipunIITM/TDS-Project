...
def analyze_data():
    ...
    """
    if not model:
        return jsonify({"error": "Gemini client is not initialized. Please check your API key."}), 500
...
Please write a Python script to answer these questions. Remember to put your final answers in a variable called 'final_answers'."""
        ...
        print("--- Generated Code by Gemini ---")
...
def health_check():
    ...
    return jsonify({"status": "Data Analyst Agent (Gemini) is running", "version": "1.0"})

@app.route("/health", methods=["GET"])
def health():
    """Additional health check endpoint."""
    return jsonify({"status": "healthy", "provider": "gemini"})
